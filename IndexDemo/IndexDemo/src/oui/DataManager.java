...
public class DataManager {
	...
	public static void CreateData(int numRows, DataPanel dp) throws Exception {
		...
		for (int i = 1; i <= numRows; i++) {
			...
			//Randomly Generating Name , UserName and Password 
			row.RollNum = i;
			row.Name = getRandomWord();
			row.UserName = getRandomWord();
			row.Password = getRandomWord();
			 
			if (canThePageAccomodateARow(pagePath, row)) {	// size if a page must me 8 kb or less
				writeTheRowInTheFile(pagePath, row.toString());	// if true then write the row in same file 
			} else {
				i--;		// else this row cannot be accomodated in current page number 
				pageNumber++;	// page number should be less than or equal to 8  
				if (pageNumber == 9) {	//if page is greater than 8 then extent no. is incremented  
					extentNumber++;		// and page numbere is reset to 1
					pageNumber = 1;
					
					dp.updateStatus((i * 100) / numRows); // progress bar is updated 
				}
			}
		}

		dp.updateStatus(100); // once process is completed bar is set to 100  
		String metadataFilePath = basedir + "\\data\\metadata";
		createFile(metadataFilePath);
		String metadata = numRows + "";		//  data file stores the no. of total no. of rows 
...
// Create Indexing --------------------------------------------------------------------------------------------
	...
	public static void Search(String columnName, String columnValue, QueryPanel qp) {
		...
		if(new File(specificIndexDirPath).exists()){
			...
			String dataDirPath = basedir + "\\data";
			String metadataFilePath = dataDirPath + "\\metadata";// storse the path to metadata file 
			int rowsTotal = 0, rowsCounter = 1, pagesCounter = 1, extentsCounter = 1;
			int columnIndex = columnName.equals("Name") ? 1 : (columnName.equals("UserName") ? 2 : 3);

			String metadataContent = readRowFromTheFile(metadataFilePath, 0);
			rowsTotal = Integer.parseInt(metadataContent);// total no. of rows are stored in mets data file 
				// rowsCounter starts from zero and goes to total rows if data is not found
			while (rowsCounter <= rowsTotal) {
				String extentPath = basedir + "\\data\\extent_" + extentsCounter;
				String pagePath = extentPath + "\\page_" + pagesCounter;
				String line = "";
				...
				try {
					...
					do {
						...
						if (line != null && line.trim().length() > 0) {
							String value = line.split("[|]")[columnIndex];// columnIndex={Name,UserName,Password}
